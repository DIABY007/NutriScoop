...
export function DeleteChallengeButton({ challengeId, challengeNom }: Props) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md p-6 rounded-2xl bg-surface shadow-xl border border-border">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-foreground">Supprimer le challenge</h3>
                <p className="text-sm text-muted mt-0.5">Cette action est irréversible.</p>
              </div>
            </div>
            <p className="text-sm text-foreground mb-6">
              Êtes-vous sûr de vouloir supprimer <strong>{challengeNom}</strong> ?
              Tous les participants et leurs suivis quotidiens seront également supprimés.
            </p>
            <div className="flex flex-col sm:flex-row-reverse gap-3">
              ...
              <button
                type="button" onClick={() => setShowConfirm(false)} disabled={pending}
